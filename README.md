<h1 align="center">Hoshi Reader Terminal</h1>

<p align="center">在终端里启动 Hoshi：书库、阅读、查词、Anki、统计和进度同步。</p>

<p align="center">
  <img alt="Python" src="https://img.shields.io/badge/Python-3.10+-3fb6e8?style=flat-square">
  <img alt="Platforms" src="https://img.shields.io/badge/Windows%20%7C%20macOS%20%7C%20Linux-terminal-5ed6b3?style=flat-square">
  <img alt="Version" src="https://img.shields.io/badge/version-0.1.0-f4c95d?style=flat-square">
  <img alt="License" src="https://img.shields.io/badge/license-MIT-d8dee9?style=flat-square">
</p>

<p align="center">
  <img src="docs/images/01-menu.svg" alt="Hoshi Reader Terminal main menu" width="760">
</p>

Hoshi Reader Terminal 是一个能在 Windows、macOS、Linux 主流终端里运行的 Hoshi Reader 恶搞项目。它参考了 [Hoshi Reader iOS](https://github.com/Manhhao/Hoshi-Reader) 和 [Hoshi Reader Android](https://github.com/HuangAntimony/Hoshi-Reader-Android) 的页面结构，把适合终端的功能做成 `hoshi` 命令。

## 功能展示

| 阅读 | 查词 |
| --- | --- |
| <img src="docs/images/02-reader.svg" alt="reader screenshot"> | <img src="docs/images/03-dictionary.svg" alt="dictionary screenshot"> |

| 同步 | 设置 |
| --- | --- |
| <img src="docs/images/04-sync.svg" alt="sync screenshot"> | <img src="docs/images/05-settings.svg" alt="settings screenshot"> |

## 已实现

- `hoshi` 直接启动中文主菜单，菜单结构参考 Hoshi：书库、查词、设置。
- 支持 `.epub`、`.txt`、`.md`、`.html`、`.xhtml` 书籍导入和阅读。
- 书库会记录阅读进度、阅读统计和划线备注。
- 支持导入 Yomitan 词典目录或 zip，带简单日语活用还原。
- 支持 Anki 挖矿：写入 CSV，也可通过 AnkiConnect 直接制卡。
- 小说目录、词典目录、同步目录都能在菜单里设置。
- 阅读进度和统计可导入/导出到本地 `ttu-reader-data` 同步目录。
- 不依赖第三方 Python 包，Python 3.10+ 即可运行。

## 安装包

仓库内已经生成三种系统的便携包：

| 系统 | 安装包 |
| --- | --- |
| Windows | [`dist/Hoshi-Reader-Terminal-0.1.0-windows.zip`](dist/Hoshi-Reader-Terminal-0.1.0-windows.zip) |
| macOS | [`dist/Hoshi-Reader-Terminal-0.1.0-macos.tar.gz`](dist/Hoshi-Reader-Terminal-0.1.0-macos.tar.gz) |
| Linux | [`dist/Hoshi-Reader-Terminal-0.1.0-linux.tar.gz`](dist/Hoshi-Reader-Terminal-0.1.0-linux.tar.gz) |

安装后打开终端输入：

```bash
hoshi
```

从源码安装：

```bash
python3 -m pip install -e .
hoshi
```

不安装也可以直接跑：

```bash
python3 -m hoshi_terminal menu
```

## 快速体验

```bash
hoshi
python3 -m hoshi_terminal 导入 examples/demo_book.txt
python3 -m hoshi_terminal 书架
python3 -m hoshi_terminal 阅读 demo --print
python3 -m hoshi_terminal 导入词典 examples/mini-yomitan
python3 -m hoshi_terminal 查词 読みました
python3 -m hoshi_terminal 挖矿 読む --sentence "端末で読むと、学習している気分だけは出る。"
python3 -m hoshi_terminal 同步 export --path ~/Documents/HoshiReaderTerminalSync
```

## 主菜单

```text
Hoshi Reader
1. 书库
2. 查词
3. 设置
0. 退出
```

书库里可以导入 EPUB、打开书架、阅读、设置小说目录。查词里可以搜索、导入辞典、查看辞典列表和设置词典目录。设置里包含辞典、Anki、外观、高级、诊断和关于。

## 阅读器按键

```text
Enter/n  下一页
p        上一页
v        切换终端纵书模式
/word    查词
a word   从当前页挖词
h note   给当前页做划线备注
s        显示本次阅读统计
g 12     跳到第 12 页
q        退出
```

## 命令

```text
menu / 菜单                    打开中文主菜单
import / 导入 PATH             导入书籍
shelf / 书架                   查看书架
read / 阅读 TARGET             阅读书籍 id、标题片段或文件路径
lookup / 查词 WORD             查词
dict-import / 导入词典 PATH    导入 Yomitan 词典 zip 或目录
mine / 挖矿 WORD               写入 CSV 或发送到 AnkiConnect
stats / 统计                   查看阅读统计
sync / 同步 [auto|export|import] 同步阅读进度
settings / 设置                打开设置
doctor / 诊断                  检查运行环境
```

## 数据目录

- Windows: `%APPDATA%\HoshiReaderTerminal`
- macOS: `~/Library/Application Support/HoshiReaderTerminal`
- Linux: `~/.local/share/hoshi-reader-terminal`

便携运行：

```bash
HOSHI_TERMINAL_HOME=.hoshi-terminal python3 -m hoshi_terminal 书架
```

## 开发

```bash
python3 -m unittest discover -s tests
python3 scripts/generate_readme_assets.py
python3 scripts/build_packages.py
```

## 致谢

菜单、阅读、辞典、Anki 和同步的轮廓参考自 Hoshi Reader iOS / Android。这个仓库没有复制上游源码，是一个终端恶搞版实现。
