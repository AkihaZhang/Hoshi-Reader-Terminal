...
- 阅读器里按 `y` 可以查看当前页匹配台词；命令行可用 `sasayaki list/play` 查看或播放。
- 音频播放优先调用 `mpv`，其次 `ffplay`；没有可 seek 的播放器时退回系统默认打开。
...
从 [GitHub Releases](https://github.com/AkihaZhang/Hoshi-Reader-Terminal/releases/tag/v0.1.7) 下载便携包。推荐用下面的一键脚本安装；zip/tar 便携包本身需要 Python 3.10+。
